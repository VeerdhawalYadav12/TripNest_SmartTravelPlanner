...
export default function TripCard({ trip, onEdit, onDelete }) {
  ...
  return (
    <div
      ...
      className={`card-hover overflow-hidden flex flex-col gap-0 p-0 transition-all duration-300 ${isActive ? 'ring-2 ring-primary-400 ring-offset-2' : ''}`}
    >
      {/* Cover gradient */}
      <div className={`h-28 bg-gradient-to-br ${COVERS[coverIdx]} flex items-end p-4 relative`}>
        ...
        {isActive && (
          <span className="bg-white/90 text-primary-700 text-xs font-semibold px-2 py-0.5 rounded-full">
            ✓ Active
          </span>
        )}
      </div>
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        ...
        <p className="text-xs text-slate-500 font-medium">{duration} day{duration !== 1 ? 's' : ''}</p>
