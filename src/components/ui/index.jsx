...
export const Button = ({
  ...
  const variants = {
    ...
    secondary: 'bg-white hover:bg-primary-50 text-primary-700 border border-primary-200 shadow-sm',
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className={`modal-content ${sizes[size]}`}>
        <div className="flex items-center justify-between p-6 border-b border-primary-100">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-primary-100 text-gray-400 hover:text-gray-600 transition-colors">
            <X size={18} />
          </button>
        </div>
...
export const Badge = ({ children, color = 'sand', className = '' }) => {
  const colors = {
    sand:   'bg-primary-100 text-primary-700',
...
export const ProgressBar = ({ value, max = 100, color = 'sand', className = '' }) => {
  ...
  return (
    <div className={`w-full bg-primary-100 rounded-full h-2 ${className}`}>
      <div
