  ...
  return (
    <div className="page-container animate-fade-in">
      ...
      {expiringSoon.length > 0 && (
        <div className="mb-6 p-4 bg-primary-50 border border-primary-200 rounded-2xl flex gap-3 animate-slide-up">
          <AlertCircle size={20} className="text-slate-500 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-semibold text-primary-700">Documents expiring soon</p>
            <p className="text-xs text-slate-500 mt-0.5">{expiringSoon.map(d => d.title).join(', ')} — renew before your trip!</p>
          </div>
        </div>
      )}
              ...
              <div key={doc.id} className="card hover:shadow-card-hover transition-all duration-300">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(doc)} className="p-1.5 rounded-lg hover:bg-primary-100 text-gray-300 hover:text-slate-500"><Pencil size={13} /></button>
                ...
                {doc.notes && <p className="mt-3 text-xs text-gray-400 bg-primary-50 rounded-lg p-2.5">{doc.notes}</p>}
                {doc.fileUrl && (
