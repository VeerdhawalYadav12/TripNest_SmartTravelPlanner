...
function ActivityItem({ item, onEdit, onDelete }) {
  ...
  const typeColors = {
    Sightseeing: 'bg-ocean-100 text-ocean-700', Food: 'bg-primary-100 text-primary-700',
  ...
  return (
    <div className="bg-white rounded-xl border border-primary-100 overflow-hidden hover:shadow-sm transition-all">
      <div className="flex items-center gap-3 p-4">
        <div className="flex flex-col items-center text-primary-400 w-12 flex-shrink-0">
          <Clock size={14} />
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => onEdit(item)} className="p-1 text-gray-300 hover:text-slate-500"><Pencil size={14} /></button>
          <button onClick={() => onDelete(item)} className="p-1 text-gray-300 hover:text-red-400"><Trash2 size={14} /></button>
        </div>
      </div>
      {expanded && item.notes && (
        <div className="px-4 pb-3"><p className="text-xs text-gray-400 bg-primary-50 rounded-lg p-2.5">{item.notes}</p></div>
      )}
    </div>
  )
}
            ...
            {Array.from({ length: days }, (_, i) => i + 1).map(day => (
              <div key={day} className="animate-fade-in">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-xl bg-primary-500 text-white flex items-center justify-center font-display font-bold text-sm flex-shrink-0">{day}</div>
                  <div>
                ...
                {byDay[day]?.length > 0 ? (
                  ...
                  <button onClick={() => openAdd(day)}
                    className="w-full border-2 border-dashed border-primary-200 rounded-xl py-4 text-sm text-primary-400 hover:border-primary-400 hover:text-slate-500 transition-colors">
