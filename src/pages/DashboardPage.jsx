  ...
  return (
    <div className="page-container animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { icon: Map,       label: 'Total Trips',     value: stats.total,                color: 'text-slate-500',   bg: 'bg-primary-50' },
      ...
      {/* Active trip banner */}
      {activeTrip && (
        <div className="mb-6 p-4 bg-gradient-to-r from-primary-500 to-primary-400 rounded-2xl flex items-center justify-between text-white animate-slide-up">
          <div>
            <p className="text-primary-100 text-xs uppercase tracking-wide font-medium mb-0.5">Currently Planning</p>
            <h3 className="font-display text-xl font-bold">{activeTrip.name}</h3>
            <p className="text-primary-100 text-sm">{activeTrip.destination} · {formatDate(activeTrip.startDate)}</p>
          </div>
        ...
        <div className="flex gap-2">
          {['all', 'upcoming', 'ongoing', 'completed'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-2 rounded-xl text-xs font-medium transition-all ${filter === f ? 'bg-primary-500 text-white' : 'bg-white text-gray-400 border border-primary-100 hover:border-primary-300'}`}>
